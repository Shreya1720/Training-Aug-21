...
SELECT FirstName+' '+LastName 'NAME' ,Salary FROM Employees WHERE Salary NOT BETWEEN 10000 AND 15000
...
WHERE DepartmentID IN (30,100) ORDER BY DepartmentID , FirstName 
...
SELECT  FirstName+' '+LastName 'NAME' ,HireDate FROM Employees WHERE YEAR(HireDate)=1987

--5. Write a query to display the first_name of all employees who have both "b" and "c" in their first name.
SELECT FirstName+' '+LastName 'NAME'  FROM Employees WHERE FirstName LIKE '%b%' AND FirstName LIKE '%c%'
...
SELECT FirstName+' '+LastName 'NAME', Salary , (0.15*Salary) 'PF' FROM Employees
